...
import com.vinay.blog.repository.PostRepo;
...
//		 
	...
	public List<PostDto> getPostByUser(Integer userId) {
		...
		User user = this.userRepo.findById(userId).orElseThrow(()-> new ResourceNotFoundException("User", "User Id", userId));
		 ...
		 List<PostDto> postDtos1 = users.stream().map((post)->this.modelMapper.map(post, PostDto.class)).collect(Collectors.toList());
		return postDtos1;
	}

	@Override
	public Post searchPosts(String key) {
